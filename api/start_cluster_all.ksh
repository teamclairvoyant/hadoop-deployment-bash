#!/bin/ksh
#
# Start Cloudera-Manager-managed environment via the REST API
#
#
...
check_root() {
  if [[ $(/usr/bin/id | awk -F= '{print $2}' | awk -F"(" '{print $1}' 2>/dev/null) -ne 0 ]]; then
    print "You must have root privileges to run this program."
    exit 2
  fi
}
...
while [[ $1 = -* ]]; do
  case $1 in
    ...
    -h|--help)
      ...
      ;;
    -v|--version)
      print '\tStart Cloudera Manager managed environment via the REST API'
      exit 0
      ;;
    *)
      print_help "$(basename "$0")"
...
echo "** Starting Cloudera Management Services..."
curl -s -X POST -u "${CMUSERNAME}:${CMPASSWORD}" "$CURLDEBUG" \
  -H "content-type:application/json" \
  "${BASEURL}/api/${API}/cm/service/commands/start" >/dev/null
echo "** Sleeping 60 seconds..."
sleep 60
...
for NAME in $CLUSTERS; do
  echo -n "** Starting Cluster ${NAME} "
  curl -s -X POST -u "${CMUSERNAME}:${CMPASSWORD}" "$CURLDEBUG" \
    -H "content-type:application/json" \
    "${BASEURL}/api/${API}/clusters/${NAME}/commands/start" >/dev/null

  while ! curl -s -u "${CMUSERNAME}:${CMPASSWORD}" "$CURLDEBUG" \
          "${BASEURL}/api/${API}/clusters/${NAME}" \
          | jq -r '.entityStatus' | grep -Eq 'STARTED|BAD_HEALTH' ; do
    now=$(date '+%s')
  ...
  echo "** Exiting Maintenence Mode for Cluster ${NAME}..."
  curl -s -X POST -u "${CMUSERNAME}:${CMPASSWORD}" "$CURLDEBUG" \
    -H "content-type:application/json" \
    "${BASEURL}/api/${API}/clusters/${NAME}/commands/exitMaintenanceMode" >/dev/null
done
