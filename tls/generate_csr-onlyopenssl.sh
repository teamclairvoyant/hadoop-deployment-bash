...
# Generate a CSR (localhost.csr) and private key (localhost.key).
...
openssl req -newkey rsa:2048 -nodes -batch -out "${_DIR}/security/x509/localhost.csr" \
  -keyout "${_DIR}/security/x509/localhost.key" -config "${_DIR}/security/x509/localhost.cnf"
chown root:root "${_DIR}/security/x509/localhost.csr"
chmod 0444 "${_DIR}/security/x509/localhost.csr"
chown root:root "${_DIR}/security/x509/localhost.key"
