...
if [ -f /opt/hortonworks/security/jks/localhost-keystore.jks ]; then
...
chmod 0440 /opt/hortonworks/security/jks/localhost-keystore.jks

chown root:hortonworks-scm /opt/hortonworks/security/jks/localhost-keystore.jks

if [ -f /opt/hortonworks/security/x509/localhost.csr ]; then
  echo "ERROR: CSR already exists.  Exiting..."
  exit 2
fi
# https://www.hortonworks.com/documentation/enterprise/5-9-x/topics/cm_sg_create_deploy_certs.html#concept_frd_1px_nw
# X509v3 Extended Key Usage:
#   TLS Web Server Authentication, TLS Web Client Authentication
# shellcheck disable=SC2086
keytool -certreq -alias localhost \
 -keystore /opt/hortonworks/security/jks/localhost-keystore.jks \
 -file /opt/hortonworks/security/x509/localhost.csr -storepass "$SP" \
 -keypass "$KP" -ext EKU=serverAuth,clientAuth -ext KU=digitalSignature,keyEncipherment $EXT

rm -f /tmp/localhost-keystore.p12.$$
keytool -importkeystore -srckeystore /opt/hortonworks/security/jks/localhost-keystore.jks \
...
if [ -f /opt/hortonworks/security/x509/localhost.e.key ]; then
...
openssl pkcs12 -in /tmp/localhost-keystore.p12.$$ -passin "pass:$KP" -nocerts \
 -out /opt/hortonworks/security/x509/localhost.e.key -passout "pass:$KP"

chmod 0400 /opt/hortonworks/security/x509/localhost.e.key

rm -f /tmp/localhost-keystore.p12.$$

if [ -f /opt/hortonworks/security/x509/localhost.key ]; then
  echo "ERROR: Key already exists.  Exiting..."
  exit 4
fi

openssl rsa -in /opt/hortonworks/security/x509/localhost.e.key \
 -passin "pass:$KP" -out /opt/hortonworks/security/x509/localhost.key

chmod 0400 /opt/hortonworks/security/x509/localhost.key
