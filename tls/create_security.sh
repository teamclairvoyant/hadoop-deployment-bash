...
print_help() {
  echo "Usage:  $1 --type <vender>"
  echo ""
  echo "        $1 -t|--type  <cloudera|hortonworks>"
  echo "        $1 [-h|--help]"
  echo "        $1 [-v|--version]"
  echo ""
  echo "   ex.  $1 --type cloudera"
  exit 1
}
...
while [[ $1 = -* ]]; do
  case $1 in
    ...
    -h|--help)
      ...
      ;;
    -v|--version)
      echo "Creates Hadoop distribution security directories."
      exit 0
      ;;
    *)
      print_help "$(basename "$0")"
...
if [ "$_TYPE" != "cloudera" ] && [ "$_TYPE" != "hortonworks" ]; then
  echo "** ERROR: --type must be one of cloudera or hortonworks."
  echo ""
  print_help "$(basename "$0")"
fi
...
# main
echo "Making TLS security directories for ${_TYPE}..."
if [ "$_TYPE" == hortonworks ]; then
  _DIR=/etc/hortonworks
elif [ "$_TYPE" == cloudera ]; then
  _DIR=/opt/cloudera
fi
mkdir -m 0755 "${_DIR}"
mkdir -m 0755 "${_DIR}/security"
mkdir -m 0755 "${_DIR}/security/x509"
mkdir -m 0755 "${_DIR}/security/jks"
mkdir -m 0755 "${_DIR}/security/CAcerts"
mkdir -m 0755 "${_DIR}/security/keytabs"
mkdir -m 0755 "${_DIR}/security/jaas"
