...
$ECHO mysql -h "$MYSQL_HOST" -u "$MYSQL_USER" "-p${MYSQL_PASSWORD}" -e "CREATE USER 'director'@'localhost' IDENTIFIED BY '$DIRECTORDB_PASSWORD';"
$ECHO mysql -h "$MYSQL_HOST" -u "$MYSQL_USER" "-p${MYSQL_PASSWORD}" -e "CREATE USER 'director'@'%' IDENTIFIED BY '$DIRECTORDB_PASSWORD';"
$ECHO mysql -h "$MYSQL_HOST" -u "$MYSQL_USER" "-p${MYSQL_PASSWORD}" -e "GRANT ALL ON director.* TO 'director'@'localhost';"
$ECHO mysql -h "$MYSQL_HOST" -u "$MYSQL_USER" "-p${MYSQL_PASSWORD}" -e "GRANT ALL ON director.* TO 'director'@'%';"
