...
echo "Installing MySQL client into Python3..."
if [ "$OS" == RedHatEnterpriseServer ] || [ "$OS" == CentOS ] || [ "$OS" == OracleServer ]; then
  ...
  yum -y -e1 -d1 install gcc openssl-devel bzip2-devel libffi-devel zlib-devel
  pip3.8 install mysqlclient
elif [ "$OS" == Ubuntu ]; then
  export DEBIAN_FRONTEND=noninteractive
  apt-get -y -q install xmlsec1 libxmlsec1-openssl libmysqlclient-dev
  pip3.8 install mysqlclient
fi
